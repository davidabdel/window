...
import { Plus, Search, ChevronRight, Clock, CheckCircle, FileText } from 'lucide-react';
...
const QuoteList: React.FC = () => {
  ...
  }, [state.quotes, state.customers, search, statusFilter]);
    ...
    <div className="p-4 space-y-4">
      ...
      {statusFilter && (
        <div className="flex items-center justify-between bg-amber-50 border border-amber-100 p-3 rounded-xl">
          <span className="text-sm font-medium text-amber-800">Showing: Needs Follow-Up</span>
          ...
          filtered.map(quote => {
            ...
            return (
              <Link 
                key={quote.id} 
                to={`/quotes/${quote.id}`}
                className={`flex flex-col gap-2 bg-white border p-4 rounded-2xl active:bg-slate-50 transition-colors shadow-sm ${
                  quote.status === 'needs-follow-up' ? 'border-amber-200 ring-1 ring-amber-100' : 'border-slate-200'
                }`}
              >
